...
#---------------------------------------------------------------------------------------------------------------------#
    ...
    ##### for CD26 ######
    ### All patient
    cds_subset <- cds[rowData(cds)$gene_short_name %in% Main_Group,]
    ...
    cds_subset_ALDH2 <- cds[rowData(cds)$gene_short_name %in% c("ALDH2"),]
    plot_genes_violin(cds_subset_ALDH2 , group_cells_by="Patient", ncol=2) +
                      theme(axis.text.x=element_text(angle=45, hjust=1))
    

    ### Test T cell
    cds_T_cell <- cds[,grepl("T cell", colData(cds)$Cell_type, ignore.case=TRUE)]
    cds_T_cell_subset <- cds_T_cell[rowData(cds_T_cell)$gene_short_name %in% Main_Group,]
    
    plot_genes_violin(cds_T_cell_subset , group_cells_by="Patient", ncol=2) +
                     theme(axis.text.x=element_text(angle=45, hjust=1))
    plot_cells(cds_T_cell,
               group_cells_by="cluster",
               cell_size=1.5,
               label_cell_groups = FALSE,
               # group_label_size = 5,
    ...
    cds_T_cell_subset_DPP4 <- cds_T_cell[rowData(cds_T_cell)$gene_short_name %in% c("DPP4"),]
    plot_genes_violin(cds_T_cell_subset_DPP4 , group_cells_by="Patient", ncol=2) +
      theme(axis.text.x=element_text(angle=45, hjust=1))

    
    ### Normal cell
    cds_Normal_cell <- cds[,grepl("N", colData(cds)$Type, ignore.case=TRUE)]
    cds_Normal_cell_subset <- cds_Normal_cell[rowData(cds_Normal_cell)$gene_short_name %in% Main_Group,]
    
    plot_genes_violin(cds_Normal_cell_subset , group_cells_by="Patient", ncol=2) +
      theme(axis.text.x=element_text(angle=45, hjust=1))
    plot_cells(cds_Normal_cell,
               group_cells_by="cluster",
               cell_size=1.5,
               label_cell_groups = FALSE,
               # group_label_size = 5,
    ...
    cds_Normal_cell_subset_DPP4 <- cds_Normal_cell[rowData(cds_Normal_cell)$gene_short_name %in% c("DPP4"),]
    plot_genes_violin(cds_Normal_cell_subset_DPP4 , group_cells_by="Patient", ncol=2) +
      theme(axis.text.x=element_text(angle=45, hjust=1))
