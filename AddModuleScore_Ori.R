...
TTT_Marker_file_Name <- c("HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION")
TTT_Marker_Name <- c("EMT")

cds <- Monocle3_AddModuleScore(TTT_Marker_file_Name,TTT_Marker_Name,marrow,cds)
plot_cells(cds, color_cells_by= TTT_Marker_Name, label_cell_groups=FALSE, show_trajectory_graph = FALSE)

library(RColorBrewer)
plot_cells(cds, color_cells_by= TTT_Marker_Name, label_cell_groups=FALSE, show_trajectory_graph = FALSE,cell_size = 1.5) +
  scale_colour_gradient2(low = "blue", mid = "grey", high = "red", guide = "colourbar")
# plot_cells(cds, color_cells_by= TTT_Marker_Name, label_cell_groups=FALSE, show_trajectory_graph = FALSE,cell_size = 1.5) +
#   scale_colour_gradient2(low="darkgreen", high="darkblue", mid="yellow", guide = "colourbar",midpoint = 0.2)
# 
# plot_cells(cds, color_cells_by= TTT_Marker_Name, label_cell_groups=FALSE, show_trajectory_graph = FALSE,cell_size = 1.5) +
#   scale_colour_gradient2(low = "blue", mid = "yellow", high = "red", guide = "colourbar",midpoint = 0.2)
plot_cells(cds, color_cells_by= TTT_Marker_Name, label_cell_groups=FALSE, show_trajectory_graph = FALSE,cell_size = 1.2) +
  scale_colour_gradient2(low = "darkblue", mid = "yellow", high = "darkred", guide = "colourbar",midpoint = 0.2)

plot_cells(cds, color_cells_by= TTT_Marker_Name, label_cell_groups=FALSE, show_trajectory_graph = FALSE,cell_size = 1.2) +
  scale_colour_gradient2(low = "#440075", mid = "#ffd261", high = "#4aff8c", guide = "colourbar",midpoint = 0.2)


plot_cells(cds, color_cells_by= TTT_Marker_Name, label_cell_groups=FALSE, show_trajectory_graph = FALSE,cell_size = 1.2) +
  scale_colour_gradient2(low = "black", mid = "#3f00bd", high = "#f691fa", guide = "colourbar",midpoint = -0.25)


plot_cells(cds, color_cells_by= TTT_Marker_Name, label_cell_groups=FALSE, show_trajectory_graph = FALSE,cell_size = 1.5)  +
  scale_colour_gradientn(colours = c("darkred", "orange", "yellow", "white"))
plot_cells(cds, color_cells_by= TTT_Marker_Name, label_cell_groups=FALSE, show_trajectory_graph = FALSE,cell_size = 1.2) +
  scale_colour_gradient2(low = "black", mid = "blue", high = "yellow", guide = "colourbar",midpoint = -0.25)


plot_cells(cds, color_cells_by= TTT_Marker_Name, label_cell_groups=FALSE, show_trajectory_graph = FALSE,cell_size = 1) +
  scale_colour_gradient2(low = "blue", mid = "grey50", high = "red") + scale_colour_gradient2(limits=c(-0.05, 0.1))
